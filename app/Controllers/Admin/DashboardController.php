...
class DashboardController extends BaseController
{
        ...
        try {
            ...
            if ($result) {
                ...
                if ($requestDetails && !empty($requestDetails['email_address'])) {
                    try {
                        $emailSent = $this->sendPaymentRequestRejectionEmail($requestDetails, $adminNotes);
                        if ($emailSent) {
                            log_message('info', "Payment request rejection email sent successfully to: {$requestDetails['email_address']}");
                        } else {
                            log_message('error', "Failed to send payment request rejection email to: {$requestDetails['email_address']} - Check SMTP configuration and logs");
                        }
                    } catch (\Exception $e) {
                        log_message('error', 'Exception while sending payment request rejection email: ' . $e->getMessage());
                        log_message('error', 'Exception trace: ' . $e->getTraceAsString());
                        // Don't fail the rejection if email fails
                    } catch (\Error $e) {
                        log_message('error', 'Error while sending payment request rejection email: ' . $e->getMessage());
                        log_message('error', 'Error trace: ' . $e->getTraceAsString());
                        // Don't fail the rejection if email fails
                    }
                } else {
                    log_message('info', 'Payment request rejection email not sent - no email address available for payer');
                }
    ...
    private function sendPaymentRequestRejectionEmail($requestDetails, $adminNotes = null)
    {
        try {
            ...
            // Format dates
            $requestDate = $requestDetails['requested_at'] ?? $requestDetails['created_at'] ?? date('Y-m-d H:i:s');
            ...
            // Build email message
            $htmlMessage = view('emails/payment_request_rejected', [
            ...
            if (!empty($brevoApiKey)) {
                try {
                    log_message('info', 'Attempting to send payment request rejection email via Brevo API to: ' . $requestDetails['email_address']);
                    ...
                    if (!class_exists('\App\Services\BrevoEmailService')) {
                        ...
                        $result = $brevoService->send($requestDetails['email_address'], $subject, $htmlMessage, $textMessage);
                        
                        if ($result['success']) {
                            log_message('info', 'Payment request rejection email sent successfully via Brevo API to: ' . $requestDetails['email_address']);
                            return true;
                        } else {
                            log_message('error', 'Brevo API failed to send payment request rejection email: ' . ($result['error'] ?? 'Unknown error') . '. Falling back to SMTP.');
                            // Fall through to SMTP attempt
                        }
                    }
                } catch (\Exception $apiException) {
                    log_message('error', 'Brevo API exception while sending payment request rejection email: ' . $apiException->getMessage() . '. Falling back to SMTP.');
                    // Fall through to SMTP attempt
                }
            }
